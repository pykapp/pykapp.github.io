---
title: Politique de modération
permalink: /fr/politique-de-moderation/
---

> **Cette page est la version française.** Elle et la [version
> anglaise](/moderation/) disent la même chose&#160;; si elles diffèrent, c'est une
> erreur de notre part.

*people you know* est chiffrée de bout en bout. Cette page dit ce que cela veut
dire pour la modération, franchement, parce que ce n'est pas ce que la plupart
des applications entendent par là.

**Dernière mise à jour&#160;:** 13 septembre 2026.

## Ce que nous ne pouvons pas faire

Nous ne pouvons voir ni les photos, ni les légendes, ni les commentaires, ni les
noms, ni les titres d'album. Ils sont chiffrés sur le téléphone qui les a faits,
avec des clés que nous ne voyons jamais. Nous ne scannons donc pas le contenu,
ne le filtrons pas et ne l'examinons pas, et nous ne le pouvons pas. Aucun
système automatique ne regarde vos photos, parce qu'il n'y a rien à regarder. Il
en va de même pour un signalement&#160;: quand vous signalez une photo, nous ne
recevons pas la photo.

## Ce que contient un signalement

Quand vous signalez une publication, un commentaire ou une personne, nous
recevons&#160;:

- qui a déposé le signalement&#160;;
- ce qu'il vise, et à quel compte cette chose appartient&#160;;
- quand&#160;;
- ce que vous avez écrit dans le champ, en clair.

Cette dernière ligne est la seule chose d'un signalement que nous puissions
lire, alors dites en mots ce qui ne va pas. Nous ne recevons jamais de copie du
contenu, et nous ne disons jamais à la personne signalée qui l'a signalée, ni
qu'un signalement existe.

## Ce que nous pouvons faire

Tout ce que nous pouvons faire agit sur un compte, jamais sur une photo&#160;:

- **Écarter** le signalement.
- **Suspendre** le compte. Un compte suspendu peut toujours se connecter,
  regarder ce qu'il a déjà, exporter ses données et se supprimer, mais ne peut
  pas publier, commenter, réagir ni envoyer de demandes. Une suspension peut
  être levée.
- **Supprimer** le compte, ce qui est la même suppression que la personne
  aurait pu faire elle-même&#160;: ses publications, ses commentaires et ses
  réactions partent pour tout le monde d'un coup, et ses relations sont
  rompues.

Nous ne pouvons pas retirer une photo et laisser le compte. Nous n'avons aucune
clé pour distinguer l'une de l'autre, et une demi-modération serait pire qu'une
modération honnête.

## Ce que vous pouvez faire vous-même, tout de suite

Les outils les plus forts sont les vôtres, et ils n'attendent pas
après nous&#160;:

- **Retirer** quelqu'un. Vous cessez tous les deux de voir ce que l'autre a
  partagé.
- **Bloquer** quelqu'un. Tout ce que retirer fait, et cette personne ne peut
  plus vous trouver, vous redemander, ni vous atteindre par l'album d'une
  tierce personne.
- **Supprimer** une de vos publications pour tout le monde.
- **Supprimer** un commentaire que quelqu'un a laissé sur votre publication.

Comme il n'y a pas d'inconnus, pas de profils publics et pas de découverte, les
gens qui peuvent vous atteindre sont des gens que vous avez choisis, et la
façon d'arrêter quelqu'un est de le déchoisir.

## Limites

Le signalement est limité en débit pour que la file ne puisse pas être noyée.
Un signalement portant sur une chose que vous ne pouvez pas voir est refusé
comme la chose elle-même le serait. Nous n'acceptons pas de signalement au
sujet d'un album, parce qu'un album est une pièce&#160;: signalez la photo qui
s'y trouve, ou la personne.

## Contact

S'il faut une personne plutôt qu'un bouton&#160;: pykapp+safety@proton.me
